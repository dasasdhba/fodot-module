...
module MetaDict =
    ...
    let getDict (node : Node) (dict : MetaDict<'a>) =
        dict |> WeakMeta.getOrAdd node (fun () -> [])
    ...
    let findOrAdd (node : Node) (predictor: 'a -> 'b option) (valueFunc : unit -> 'a * 'b) (dict : MetaDict<'a>)  =
        dict
        |> getDict node
    ...
    let getOrAdd (node : Node) key (predictor: 'a -> 'b option) (valueFunc : unit -> 'a * 'b) (dict : MetaDict<'a>) =
        dict
        |> getDict node
